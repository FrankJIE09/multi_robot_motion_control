...
def get_tf_matrix():
    ...
    try:
        ...
        rospy.loginfo("Got transform matrix:\n%s", T)
        return T
...
def get_target_pose(target_in_base_link):
    ...
    # 齐次形式表示 base_link 下的目标点
    point_vehicle = np.array([target_in_base_link[0], target_in_base_link[1], target_in_base_link[2], 1.0])
    ...
    rospy.loginfo(f"生成的目标 PoseStamped:\n{target_pose}")
    return target_pose

if __name__ == "__main__":
    rospy.init_node("test")
    target_in_base_link = [0.4, 0.2, 0.0]
    target_pose = get_target_pose(target_in_base_link)
    if target_pose:
        sagepy.move_arc(target_pose_stamped= target_pose,
                control_frame="map",
                ...
                free_speed = False,
                stop_distance=0.0)
        sagepy.turn(target_pose=target_pose.pose,theta_tolerance=0.01,total_time=0.005)